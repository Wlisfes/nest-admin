	...
	setup() {
		...
		return () => (
			<n-el tag="div" class="app-nav">
				{trigger.value && <u-icon name="antd-left" style={{ marginRight: '10px' }}></u-icon>}
				...
				{trigger.value && <u-icon name="antd-right" style={{ marginLeft: '10px' }}></u-icon>}
				<n-dropdown
					options={[
						{ label: '刷新当前', key: 'reload', icon: 'antd-reload' },
						{ label: '关闭当前', key: 'close-current', icon: 'antd-close', disabled: disabled.value },
						{ label: '关闭其他', key: 'close-other', icon: 'antd-column-width', disabled: disabled.value },
						{ label: '关闭全部', key: 'close-all', icon: 'antd-line', disabled: disabled.value }
					]}
					render-icon={(u: { icon: string }) => <u-icon name={u.icon}></u-icon>}
					show-arrow
					on-select={onSelecter}
				>
					<u-icon name="antd-menu" style={{ marginLeft: '10px' }}></u-icon>
				</n-dropdown>
			</n-el>
		)
	}
